...
SEARCH_PATH = "/mnt/home/husiyuan/code/mtp-go_me/data_sets/highD"
INPUT_LENGTH = 2
PRED_HORIZON = 5
N_IN_FEATURES = 9
N_OUT_FEATURES = 7
...
def add_maneuver(tracks_meta, tracks):
    ...
    t_ids = pd.unique(tracks.trackId)
    for t_id in t_ids:
        n_lane_changes = int(tracks_meta[tracks_meta.trackId == t_id].numLaneChanges.iloc[0])
...
if __name__ == "__main__":
    ...
    for r_id in rec_ids:
        ...
        rec_meta = pd.read_csv(f'{SEARCH_PATH}/{r_id}_recordingMeta.csv')
        ...
        for i in ii:
            ...
            n_lane_changes = int(t_meta[t_meta.trackId == id0].numLaneChanges.iloc[0])
            if n_lane_changes == 0:
                if id0 in seen_ids:
                    continue
                else:
            ...
            driving_dir = int(t_meta[t_meta.trackId == id0].drivingDirection.iloc[0])
