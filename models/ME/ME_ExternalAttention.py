...
class ExternalAttention(nn.Module):
    ...
    def forward(self, queries):  # torch.Size([32, 784, input最后一维度128])
...
if __name__ == '__main__':
    input = torch.rand(993, 9 ,4).cuda()
    block = ExternalAttention(d_model=input.shape[-1], S=8).cuda()
    # input = torch.rand(32, 784, 128).cuda()
    
    output = block(input)   # 不变
    print(input.size())
    print(output.size())
