...
过度平滑是各种图神经网络 (GNN) 和 Transformer 中的常见现象，其性能会随着层数的增加而下降。
我们不是从表示收敛到单个点的完全崩溃的角度来描述过度平滑，而是深入研究维度崩溃的更一般视角，其中表示位于一个狭窄的锥体中。
因此，受到对比学习在防止维度崩溃方面的有效性的启发，我们提出了一种称为 ContraNorm 的新型规范化层。直观地说，ContraNorm 隐式地破坏了嵌入空间中的表示，从而导致更均匀的分布和更轻微的维度崩溃。
在理论分析中，我们证明了 ContraNorm 在某些条件下可以缓解完全崩溃和维度崩溃。
我们提出的规范化层可以轻松集成到 GNN 和 Transformer 中，并且参数开销可以忽略不计。
