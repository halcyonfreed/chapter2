...
"""xLSTM：扩展长短期记忆
20 世纪 90 年代，恒定误差轮播和门控被引入作为长短期记忆 (LSTM) 的核心思想。
从那时起，LSTM 经受住了时间的考验，并为众多深度学习成功案例做出了贡献，特别是它们构成了第一个大型语言模型 (LLM)。
...
首先，我们引入指数门控，并采用适当的规范化和稳定技术。其次，我们修改 LSTM 内存结构，
获得：(i) 具有标量内存、标量更新和新内存混合的 sLSTM，
(ii) 具有矩阵内存和协方差更新规则的完全可并行的 mLSTM。
...
if __name__ == '__main__':
    ...
    # 运行前向传递 [batch_size, seq_length, hidden_size]
